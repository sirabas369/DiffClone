...
class DMAgent(Agent):
    def __init__(self, models, learning_rate, device, out_dim, H=1):
        ...
        self.noise_scheduler = DDPMScheduler(num_train_timesteps=50, beta_schedule="squaredcos_cap_v2", prediction_type='epsilon') 
        ...
        self.learning_rate_scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=[15,30], gamma=0.5)
        self.device = device
        self.epoch = 0
    ...
    def sampling(self, inp):
        return self.models['decoder'].sampling(inp, self.inference_scheduler, self.H)
...
class Policy(nn.Module):
    def __init__(self, inp_dim, out_dim, H, hidden_dim=128, device="cuda"):
        ...
        self.model = transformer_encoder(condition_dim=inp_dim, out_dim=out_dim, hidden_dim=hidden_dim, H=H, device=device)
        self.device = device
    ...
    def forward(self, samples, noise_scheduler, H, epoch):
        ...
        timesteps = torch.randint(0, noise_scheduler.num_train_timesteps, (bs,1), device=self.device)
        noisy_h = noise_scheduler.add_noise(h, noise, timesteps)

        pred = self.model(noisy_h, timesteps.float(), condition=observations)
...
def _init_agent_from_config(config, device='cpu', normalization=None):
    ...
    bc_agent = DMAgent(models, config.training.lr, device, config.data.out_dim, H)
